...
def deployReferenceAppCreateBlobstore(config):
	try:
		print("****************** Running deployReferenceAppCreateBlobstore ******************")
		config.current='deployReferenceAppCreateBlobstore'
		blobstoreJsonrequest = "cf cs "+config.predixBlobstore+" "+config.predixBlobstorePlan +" "+config.rmdBlobstore
		createService(config.rmdBlobstore,blobstoreJsonrequest)
	except:
		print(traceback.print_exc())
		print()
		print ('Exception when running ' + config.current + '.  Retrying')
...
def configureManifest(config, manifestLocation):
	...
	if config.useDataexchangeBlobStore in ("y","Y","Yes","Yes") :
		if hasattr(config,'rmdBlobstore') :
			print ('Replacing blobStoreService')
			s = s.replace('{blobStoreService}', config.rmdBlobstore)
			s = s.replace('#blobservice_name :', 'blobservice_name :')
