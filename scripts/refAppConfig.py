...
global resolver
global user
global useDataexchangeBlobStore
global useDataexchangeRabbitMQ
try:
    #set defaults
    useDataexchangeBlobStore="n"
    useDataexchangeRabbitMQ="n"
    instanceAppender = ""
    ...
    opts, args = getopt.getopt(sys.argv[1:],"d:e:i:s:p:r:a:v:c:o:r:f:x:y:z:",["useblobstore=","userabbitmq=","delete=","environment=","instanceAppender=","mvnsettings=","pullsubmodules=","mavenrepo=","continueFrom=","only=", "fastinstall=", "artifactoryrepo=", "artifactoryuser=", "artifactorypass="])
...
for opt, arg in opts:
    ...
    if opt == '-h':
        ...
        artifactorypass = arg;
    elif opt in ("-r","--userabbitmq"):
        useDataexchangeRabbitMQ = arg;
    elif opt in ("-b","--useblobstore"):
        useDataexchangeBlobStore = arg;
...
if len(instanceAppender) == 0:
    instanceAppender = user.strip().split("@")[0].replace('.', '-')
...
if environment == 'PROD' or environment == 'SELECT' or environment == 'JAPAN' or environment == 'UK' or environment == 'FREE' :
    ...
    predixBlobstore = "predix-blobstore"
    ...
    predixRedisPlan = "shared-vm"
    predixBlobstorePlan = "Tiered"
elif environment == 'HACK' :
    # Predix Service Instance Name for sysint
    predixUaaService = "predix-uaa-training"
    predixAcsService = "predix-acs"
    predixAssetService = "predix-asset"
    predixTimeSeriesService = "predix-timeseries"
    predixPostgres = "rdpg"
...
if environment == 'JAPAN':
    resolver = "10.120.0.2"

if environment == 'UK':
    resolver = "10.32.99.10"
